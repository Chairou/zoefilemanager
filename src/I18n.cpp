...
const QHash<QString, QString>& zhDict() {
    static const QHash<QString, QString> kZh = {
        ...
        {"Share:",                  "共享名："},
        {"Workgroup:",              "工作组："},
        {"share name, e.g. Public", "共享名，例如 Public"},
        {"optional Windows workgroup/domain", "可选：Windows 工作组/域名"},
        {"Missing share",           "缺少共享名"},
        {"Please enter the SMB share name (e.g. Public).",
            "请输入 SMB 共享名（例如 Public）。"},
        {"SMB coming soon",         "SMB 即将支持"},
        {"SMB/Samba support is being wired up. The UI accepts your "
         "parameters but the connection backend is not yet active.",
            "SMB/Samba 支持正在接入中。当前界面已可收集参数，但底层连接暂未启用。"},
        {"password (leave empty for guest)",
            "密码（匿名/guest 共享可留空）"},
        {"username (try DOMAIN\\\\user or user@domain in AD environments)",
            "用户名（企业 AD 域环境请用 域\\用户 或 用户@域 格式）"},
        {"home/SOHO: leave empty; corporate AD: enter your domain (e.g. TENCENT)",
            "家用/SOHO 留空；企业 AD 域填大写域名（如 TENCENT）"},
        {"hostname or IP",          "主机名或 IP"},
        {"username",                "用户名"},
